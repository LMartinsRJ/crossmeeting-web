...
export default function GranolaCard({ cred }: { cred: Cred }) {
  ...
  async function save() {
    ...
    try {
      ...
      setMsg({ text: 'Granola conectado!', ok: true })
      setKey('')
  ...
  async function sync() {
    ...
    try {
      ...
      const json = await res.json()
      if (!res.ok) { setMsg({ text: json.error, ok: false }); return }
      setMsg({ text: `${json.imported} nota(s) importada(s) de ${json.total} encontradas.`, ok: true })
    } finally {
      setSyncing(false)
    }
  }
  ...
  async function disconnect() {
    await fetch('/api/integrations/granola/connect', { method: 'DELETE' })
    ...
    <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-5">
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <p className="text-sm font-medium text-white">Granola</p>
            {connected ? (
          ...
          <p className="text-xs text-neutral-500 mt-0.5 leading-relaxed">
            Importe notas e transcrições do Granola via API oficial.
            {!connected && ' Gere sua chave em Granola → Settings → Connectors → API keys.'}
          </p>
      ...
      {!connected ? (
        <div className="flex gap-2">
          <input
            type="password"
            placeholder="grn_xxxxxxxxxxxxxxxxxxxx"
            value={key}
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-4 py-2 rounded-xl bg-[#6C8EFF]/20 hover:bg-[#6C8EFF]/30 text-[#6C8EFF] text-xs font-medium border border-[#6C8EFF]/30 disabled:opacity-40 transition-colors"
          >
            {syncing ? 'Sincronizando…' : 'Sincronizar Granola'}
          </button>
          <button
            onClick={disconnect}
