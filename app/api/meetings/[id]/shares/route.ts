...
async function getOwnedMeeting(service: ReturnType<typeof getService>, meetingId: string, ownerId: string) {
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const meeting = await getOwnedMeeting(service, id, profile.id)
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (normalizedEmail === user.email.toLowerCase()) {
    return NextResponse.json({ error: 'Você já tem acesso a esta reunião.' }, { status: 400 })
  }
  ...
  const { data: share, error } = await service
    ...
    }, { onConflict: 'meeting_id,shared_with_email' })
    .select()
    .single()
